...
public class Book {
    ...
    public Book(String isbn, String title, int totalCopies, Money price, LocalDate publicationDate,
    ...
    public void addAuthor(Author author, AuthorRole role) {
        BookAuthor ba = new BookAuthor(this, author, role);
        bookAuthors.add(ba);
        author.addBookAuthorInternal(ba);
    }
    ...
    public void incrementAvailableCopies() {
        if (availableCopies >= totalCopies)
            throw new IllegalStateException("Available copies cannot exceed total copies for: " + isbn);
        availableCopies++;
    }

    public boolean isAvailable() { return availableCopies > 0; }

    public Optional<Author> getPrimaryAuthor() {
        return bookAuthors.stream()
            .filter(ba -> ba.getRole() == AuthorRole.PRIMARY)
